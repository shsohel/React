...
import { Dropdown } from 'customControls';
import { ResetButton, SubmitButton } from 'customControls/ActionButtons/ActionButtons';
...
export default function ApprovalProcessForm(props) {
  ...
  async function getAllApprovalProcess() {
    await axios
      .get(`${urls.utilities.get_approval_process_type_list}`)
      .then(res => {
  ...
  async function getAllUser() {
    await axios
      .get(`${urls.account.users.get_all_users}`)
      .then(res => {
        const body = res.data;
        setUsers(
          body.map(item => ({
        ...
        <Grid container item xs={12} sm={12} md={12} lg={12} justify="space-around">
          <Grid item xs={12} sm={12} md={12} lg={12}>
            <Dropdown
              ...
              options={processType.filter(item => item.value === 'OrderSubmit')}
              // options={processType}
            />
          </Grid>
          <Grid item xs={12} sm={12} md={12} lg={12}>
        ...
        <Grid container item xs={12} sm={12} md={12} lg={12} justify="flex-end">
          <div>
            <SubmitButton
              ...
              text="Submit"
            />
            <ResetButton text="Reset" color="default" onClick={resetForm} />
          </div>
        </Grid>
      </GridContainer>
    </Form>
