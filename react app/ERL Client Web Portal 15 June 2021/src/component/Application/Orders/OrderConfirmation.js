...
import { ViewIcon } from 'customControls/ActionButtons/ActionButtons';
...
export default function OrderConfirmation() {
  ...
  const getAllPendingOrders = async () => {
    try {
      await axios.get(`${urls.order.get_pending_orders}`).then(({ data }) => {
        if (data.succeeded) {
  ...
  const getAllApprovalOrders = async () => {
    try {
      await axios.get(`${urls.order.get_approved_orders}`).then(({ data }) => {
        if (data.succeeded) {
          const body = data.data;
  ...
  const PreviewOrder = key => {
    if (key) {
      axios.get(`${urls.order.get_by_key}/${key}`).then(({ data }) => {
        if (data.succeeded) {
          const body = data.data;
    ...
    <div>
      <PageContainer heading="Order Confirmation Process" breadcrumbs={breadcrumbs}>
        <div className={classes.mainDiv}>
          ...
          {recordForDetails ? (
            <PopupSection title="Order Details " openPopup={openPopup} setOpenPopup={setOpenPopup}>
